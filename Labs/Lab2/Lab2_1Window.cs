...
namespace Labs.Lab2
{
    ...
    {        
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            float[] vertices = new float[] { -0.8f, 0.8f, 0.4f, 0.0f, 1.0f, 0.0f,
                                             -0.6f, -0.4f, 0.4f, 1.0f, 0.0f, 0.0f,
            ...
            {
                throw new ApplicationException("Triangle vertex data not loaded onto graphics card correctly");
            }
            ...
            {
                throw new ApplicationException("Triangle index data not loaded onto graphics card correctly");
            }

            GL.GenBuffers(2, mSquareVertexBufferObjectIDArray);
            float[] squareVert = new float[] { -0.2f, -0.4f, 0.2f, 0.0f, 0.3f, 1.0f,
                                               0.8f, -0.4f, 0.2f, 0.0f, 0.6f, 0.6f,
                                               0.8f, 0.6f, 0.2f, 0.0f, 0.0f, 1.0f,
                                               -0.2f, 0.6f, 0.2f, 0.0f, 0.7f, 0.3f};
            ...
            GL.GetBufferParameter(BufferTarget.ArrayBuffer, BufferParameterName.BufferSize, out size);
            if (squareVert.Length * sizeof(float) != size)
            {
                throw new ApplicationException("Square vertex data not loaded onto graphics card correctly");
            }
            ...
            GL.GetBufferParameter(BufferTarget.ElementArrayBuffer, BufferParameterName.BufferSize, out size);
            if (squareIndices.Length * sizeof(float) != size)
            {
                throw new ApplicationException("Square index data not loaded onto graphics card correctly.");
            }
